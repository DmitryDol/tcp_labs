...
class SQLAlchemyRepository(AbstractRepository):
    ...
    def _get_primary_key_columns(self) -> List:
        ...
        if hasattr(self.model, '__table__') and hasattr(self.model.__table__, 'primary_key'):
            return [getattr(self.model, c.name) for c in self.model.__table__.primary_key.columns]
        return [getattr(self.model, 'id')]
    
    async def add_one(self, data: Dict[str, Any]) -> Union[Dict[str, Any], int]:
        """
    ...
    async def edit_one(self, id_or_filter: Union[int, Dict[str, Any]], data: Dict[str, Any]) -> Union[Dict[str, Any], int, None]:
        """
        ...
        if isinstance(id_or_filter, dict):
            stmt = update(self.model).values(**data).filter_by(**id_or_filter).returning(*pk_columns)
        else:
            stmt = update(self.model).values(**data).filter_by(id=id_or_filter).returning(*pk_columns)
    ...
    async def find_all(self, filter_by: Optional[Dict[str, Any]] = None) -> List[Any]:
        """
    ...
    async def delete_one(self, id_or_filter: Union[int, Dict[str, Any]]) -> Union[Dict[str, Any], int, None]:
        """
