...
from dto import LoginDTO, TokenDTO, UserAddDTO
...
async def create_user(user: UserAddDTO, uow: UOWDep):
...
@router.post("/token", response_model=LoginDTO)
...
async def refresh_token(
    ...
    if access_token:
        ...
        access_token_expires_at = datetime.fromtimestamp(
            access_token_payload.get("exp"), tz=UTC
        )
    ...
    if access_token:
        await TokensService.revoke_token(
            access_token_jti, access_token_expires_at, redis_dep
        )
...
@router.get("/me")
async def get_curr_user(user_dep: UserDep):
    return user_dep
